public class Main {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < data.length; i++) System.out.printf("%d: (Binary) %s (Octal) %s (Hexax) %s (Neeraj) %s\n", data[i], binary(data[i]), octal(data[i]), hexaDecimal(data[i]), NeerajSeries(data[i]));
    }
    ...
    //base 36 0-9, A-Z
    public static String NeerajSeries(int decimal) {
        int[]  NeerajSeries = {0, 0, 0, 0, 0, 0, 0, 0, 0};
        if (decimal < 36) NeerajSeries[8] = decimal;
        if (decimal >= 36) {
            for (int i = NeerajSeries.length - 1; i >= 0; i--) {
                NeerajSeries[i] = decimal % 36;
                decimal /= 36;
            } }
        String build = converter(NeerajSeries);
        return build;
    }//base 36
